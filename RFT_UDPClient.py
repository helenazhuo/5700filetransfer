...
class RFT_UDPClient:
    ...
    def request_file(self, fn: str, dest_ip: str, dest_port: int):
        ...
        # Create dir for file
        save_path = os.path.join('saved_files', f'client_{self.client_id}', fn)
        os.makedirs(os.path.dirname(save_path), exist_ok=True)

        # Start threads for receiving file packets and sending acks
        self.stop_ack = threading.Event()
        recv_t = threading.Thread(target=self.receive_file, args=(save_path,))
    ...
    def receive_file(self, save_path):
        ...
        Args:
            save_path (str): Path to write the received file to.
        """
        received = {} # Tracks received chunks, indexed by seq_num.
        with open(save_path, 'wb') as f: # open in binary mode
            while True:
